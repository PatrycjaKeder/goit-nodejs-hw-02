...
router.post(
  ...
  cntrlTryCatchWrapper(register)
);
router.get("/verify/:verificationToken", cntrlTryCatchWrapper(verifyEmail));
router.post(
  "/verify",
  validateBody(verifyEmailSchema),
  cntrlTryCatchWrapper(resendVerifyEmail)
);
router.post(
  "/login",
  isEmptyBody,
  validateBody(logInSchema),
  cntrlTryCatchWrapper(login)
);
...
router.patch(
  ...
  cntrlTryCatchWrapper(patchSubscription)
);
router.patch(
  "/avatars",
  authenticate,
  upload.single("avatar"),
  cntrlTryCatchWrapper(updateAvatar)
);
